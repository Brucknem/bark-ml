...
class GAILRunner(TFARunner):
  ...
  def _train(self):
    ...
    for _ in range(0, self._params["ML"]["GAILRunner"]["NumberOfCollections", "", 10000]):
      self._agent._training = True
      global_iteration = self._agent._agent._train_step_counter.numpy()
      self._collection_driver.run()
      experience, _ = next(iterator)
      self._agent._agent.train(experience)
      if global_iteration % self._params["ML"]["GAILRunner"]["EvaluateEveryNSteps", "", 100] == 0:
        self.Evaluate()
        self._agent.Save()